...
(function () {
    ...
    UploadManager.prototype.put = function (options, callback) {
        ...
        if (!!options && !!options.path && !!options.name) {
            ...
            var relative_path = path.join(path.storagepath(), ly.replaceAll(' ', '_', options.name));
            var absolute_path = path.join(self.root, relative_path);
            var overwrite = null != options.overwite ? options.overwrite : true;
            copyFile(this, temp_location, absolute_path, overwrite, function(err, file){
